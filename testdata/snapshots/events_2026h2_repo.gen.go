...
type EventsV2026H2Filter struct {
	ID            *int64
	IDIn          []int64
	IDGt          *int64
	IDLt          *int64
...
func eventsV2026H2sFilterWhere(f EventsV2026H2Filter) []pgb.Expr {
	...
	if len(f.IDIn) > 0 {
		w = append(w, EventsV2026H2s.ID().In(f.IDIn...))
	}
	if f.IDGt != nil {
		w = append(w, EventsV2026H2s.ID().Gt(*f.IDGt))
	}
	if f.IDLt != nil {
		w = append(w, EventsV2026H2s.ID().Lt(*f.IDLt))
	}
	if f.IDGte != nil {
		w = append(w, EventsV2026H2s.ID().Gte(*f.IDGte))
	}
	if f.IDLte != nil {
		w = append(w, EventsV2026H2s.ID().Lte(*f.IDLte))
	}
	if f.UserID != nil {
		w = append(w, EventsV2026H2s.UserID().Eq(*f.UserID))
	}
	if len(f.UserIDIn) > 0 {
		w = append(w, EventsV2026H2s.UserID().In(f.UserIDIn...))
	}
	if f.UserIDGt != nil {
		w = append(w, EventsV2026H2s.UserID().Gt(*f.UserIDGt))
	}
	if f.UserIDLt != nil {
		w = append(w, EventsV2026H2s.UserID().Lt(*f.UserIDLt))
	}
	if f.UserIDGte != nil {
		w = append(w, EventsV2026H2s.UserID().Gte(*f.UserIDGte))
	}
	if f.UserIDLte != nil {
		w = append(w, EventsV2026H2s.UserID().Lte(*f.UserIDLte))
	}
	if f.Kind != nil {
		w = append(w, EventsV2026H2s.Kind().Eq(*f.Kind))
	}
	if len(f.KindIn) > 0 {
		w = append(w, EventsV2026H2s.Kind().In(f.KindIn...))
	}
	if f.KindLike != nil {
		w = append(w, EventsV2026H2s.Kind().Like(*f.KindLike))
	}
	if f.KindILike != nil {
		w = append(w, EventsV2026H2s.Kind().ILike(*f.KindILike))
	}
	if f.Payload != nil {
		w = append(w, EventsV2026H2s.Payload().Eq(*f.Payload))
	}
	if f.OccurredAt != nil {
		w = append(w, EventsV2026H2s.OccurredAt().Eq(*f.OccurredAt))
	}
	if len(f.OccurredAtIn) > 0 {
		w = append(w, EventsV2026H2s.OccurredAt().In(f.OccurredAtIn...))
	}
	if f.OccurredAtGt != nil {
		w = append(w, EventsV2026H2s.OccurredAt().Gt(*f.OccurredAtGt))
	}
	if f.OccurredAtLt != nil {
		w = append(w, EventsV2026H2s.OccurredAt().Lt(*f.OccurredAtLt))
	}
	if f.OccurredAtGte != nil {
		w = append(w, EventsV2026H2s.OccurredAt().Gte(*f.OccurredAtGte))
	}
	if f.OccurredAtLte != nil {
		w = append(w, EventsV2026H2s.OccurredAt().Lte(*f.OccurredAtLte))
	}
	w = append(w, f.Extra...)
	return w
}

// EventsV2026H2Set is the three-state UPDATE payload: a zero Set field
// skips its column, SetOf writes a value, SetNull writes NULL.
type EventsV2026H2Set struct {
	UserID     Set[int64]
	Kind       Set[string]
	Payload    Set[[]byte]
...
// InsertEventsV2026H2Params carries the INSERT column values: generated
// and serial columns are out, default-bearing columns join only with
// the include_defaults option.
type InsertEventsV2026H2Params struct {
	UserID     int64
	Kind       string
	Payload    []byte
	OccurredAt pgtype.Timestamptz
...
// scanEventsV2026H2 scans one row positionally over every column in
// catalog order — the single scan path for all generated reads.
func scanEventsV2026H2(row pgx.CollectableRow) (EventsV2026H2, error) {
	var m EventsV2026H2
...
func ListEventsV2026H2s(ctx context.Context, exec pgb.DBTX, f EventsV2026H2Filter, opts ...pgb.ListOpt) ([]EventsV2026H2, error) {
	...
	return pgx.CollectRows(rows, scanEventsV2026H2)
}
...
func InsertEventsV2026H2(ctx context.Context, exec pgb.DBTX, p InsertEventsV2026H2Params) (EventsV2026H2, error) {
	...
	us, err := pgx.CollectRows(rows, scanEventsV2026H2)
	if err != nil {
		return EventsV2026H2{}, err
	}
	if len(us) == 0 {
...
const insertEventsV2026H2sSQL = "INSERT INTO public.events_2026h2 (user_id, kind, payload, occurred_at) SELECT * FROM unnest($1::int8[], $2::text[], $3::jsonb[], $4::timestamptz[]) RETURNING id, user_id, kind, payload, occurred_at"
...
func InsertEventsV2026H2s(ctx context.Context, exec pgb.DBTX, ps []InsertEventsV2026H2Params) ([]EventsV2026H2, error) {
	...
	rows, err := exec.Query(ctx, insertEventsV2026H2sSQL, args...)
	if err != nil {
		return nil, err
	}
	return pgx.CollectRows(rows, scanEventsV2026H2)
