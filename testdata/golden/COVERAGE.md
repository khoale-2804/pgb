...
- Fixture correction (2026-09-14): users.bitfield's `DEFAULT B'0'` was invalid for bit(8) (SQLSTATE 22026 on first use) — it had never been exercised because inserts always bound the column explicitly. Now `DEFAULT B'00000000'`. Default-bearing columns are omitted from INSERT lists unless `include_defaults` is set, so server defaults are actually executed by the suite.
